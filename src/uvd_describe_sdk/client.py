...
from .hashes import SETTLEMENT_PENDING, looks_like_onchain_id
from .models import (
    AgentReputation,
    Breakdown,
    IndexHealth,
...
class DescribeClient:
    ...
    def _server_reason(response: httpx.Response) -> Optional[str]:
        ...
        * **URLs are redacted first** (`[a-z][a-z0-9+.-]*://…` →
          `[url-redacted]`, the same token the TS twin writes — one text, two twins). The body is written by the SERVER: a 5xx can echo
    ...
    def _receipt(response: httpx.Response) -> PaymentReceipt:
        ...
        `pending` is a LEGITIMATE value the OpenAPI declares ("settlement has not
        reported one") and it is not marked as malformed — that part holds.

        🔴 CORRECTION (2026-08-31): what did NOT hold was letting the literal
        travel INSIDE `transaction_hash`. Until 0.1.0 this method returned
        `transaction_hash="pending"` on a 200, indistinguishable from a citable
        hash unless every caller re-checked the string — the exact family of
        Execution Market's INC-2026-08-26, a placeholder occupying the place of
        the proof. Now the sentinel is peeled off FIRST: it becomes
        `transaction_hash=None` + `settlement_pending=True`, so the hash field
        only ever carries something shaped like a hash, and `None` keeps meaning
        what it means everywhere else — no proof either way. The old rule lived
        in `hashes.looks_like_settlement_receipt`, which still exists for anyone
        validating the raw header by hand.
        """
        crudo = response.headers.get("X-Payment-Receipt")
        malos: List[str] = []
        recibo: Optional[str] = None
        pendiente = crudo is not None and str(crudo) == SETTLEMENT_PENDING
        if crudo is not None and not pendiente:
            if looks_like_onchain_id(str(crudo)):
                recibo = str(crudo)
            else:
